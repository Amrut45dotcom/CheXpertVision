...
def get_optimizer(model, name):
    ...
    if name == "densenet121":
        ...
        backbone_params = [p for p in model.parameters()
                           if not any(p is cp for cp in classifier_params)]
    else:
        raise ValueError(f"Differential LR not configured for {name}")
...
def build_scheduler(optimizer, warmup_steps, t_max, steps_per_epoch):
    """500-step linear warmup → CosineAnnealingLR over remaining steps"""
    warmup_epochs = warmup_steps / steps_per_epoch  # fractional OK for LinearLR
    warmup_scheduler = LinearLR(
        optimizer,
        start_factor=1e-3,
        end_factor=1.0,
        total_iters=warmup_steps   # step-level
    )
    cosine_scheduler = CosineAnnealingLR(optimizer, T_max=t_max)
    # Switch after warmup_steps batches → approximate with epochs
    warmup_epoch_count = max(1, round(warmup_epochs))
...
def main():
    ...
    # Full dataset — no 50% patient filter
    ...
    scheduler = build_scheduler(optimizer, WARMUP_STEPS, T_MAX, steps_per_epoch=len(train_loader))
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        print(f"\nEpoch {epoch+1}/{MAX_EPOCHS} | "
              ...
              f"Mean AUC: {mean_auc:.4f} | Time: {duration:.2f} min")
